...
export const BASE_API_URL = (() => {
  ...
  if (!url) {
    logger.error('VITE_BASE_API_URL is not defined in environment variables.');
  ...
  try {
    ...
    if (parsed.protocol !== 'https:') {
      logger.warn('VITE_BASE_API_URL does not use HTTPS. Ensure secure transport.');
    }
    return parsed.toString();
  } catch (e) {
    logger.error('VITE_BASE_API_URL is not a valid URL', { error: e });
...
export const SLUG_URL = (() => {
  ...
  if (!url) {
    logger.error('VITE_SLUG_URL is not defined in environment variables.');
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production' && parsed.protocol !== 'https:') {
      logger.warn('VITE_SLUG_URL is not using HTTPS in production.');
    }
    return parsed.toString();
  } catch (e) {
    logger.error('VITE_SLUG_URL is not a valid URL', { error: e });
...
export const ORG_BASE_API_URL = (() => {
  ...
  if (!url) {
    logger.error('VITE_ORG_BASE_API_URL is not defined in environment variables.');
    throw new Error('Missing required environment variable: VITE_ORG_BASE_API_URL');
  }
  try {
    const parsed = new URL(url);
    if (parsed.protocol !== 'https:') {
      logger.warn('VITE_ORG_BASE_API_URL does not use HTTPS. Ensure secure transport.');
    }
    return parsed.toString();
  } catch (e) {
    logger.error('VITE_ORG_BASE_API_URL is not a valid URL', { error: e });
