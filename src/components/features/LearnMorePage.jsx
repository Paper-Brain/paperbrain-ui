...
const CTA_BUTTON_CLASSES = 
  ...
  "text-blue-800 text-sm tracking-wider hover:bg-emerald-300 " +
  "transition-colors duration-300 flex items-center justify-center mx-auto group";

const CARD_LINK_CLASSES = 
  "block w-full px-6 py-3 bg-gradient-to-r from-purple-400 to-yellow-300 " +
  "text-blue-800 text-sm tracking-wider hover:bg-emerald-300 " +
  "transition-colors duration-300 text-center group";

function ResourceCard({ resource }) {
  return (
    <div className="p-8 border border-gray-800 hover:border-violet-400/50 transition-all duration-500 group">
    ...
    <div className="bg-[#0A0A0A] text-white selection:bg-violet-400 selection:text-black">
      <div className="pt-32 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <HeroSection />
          <ResourcesGrid />
          <CTASection onClick={handleContactSupport} />
        </div>
      </div>
    </div>
  );
