...
function FrameworkSelect({
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute w-full sm:w-48 mt-2 bg-[#0A0A0A] border border-white/10 rounded-md shadow-lg z-10">
          {FRAMEWORKS.map((fw) => (
            <button
              key={fw}
...
function OutputConsole({ output }: { output: string }) {
  ...
  return (
    <div className="mt-6">
      <h3 className="text-sm text-gray-400 font-normal mb-2">
        Output Console:
      </h3>
...
function generateTestOutput(framework: string): string {
